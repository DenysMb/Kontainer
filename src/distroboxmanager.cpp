...
bool DistroboxManager::enterContainer(const QString &name)
{
    ...
    QString command = QStringLiteral("konsole --workdir %1 -e distrobox enter %2").arg(homeDir, name);
...
bool DistroboxManager::upgradeContainer(const QString &name)
{
    ...
    QString command = QStringLiteral("konsole --workdir %1 -e bash -c '%2'").arg(homeDir, upgradeCmd);
...
QString DistroboxManager::getDistroColor(const QString &image)
{
    ...
    return QStringLiteral("#%1%2%3").arg(r, 2, 16, QLatin1Char('0')).arg(g, 2, 16, QLatin1Char('0')).arg(b, 2, 16, QLatin1Char('0'));
}
...
bool DistroboxManager::installPackageInContainer(const QString &name, const QString &packagePath, const QString &image)
{
    ...
    if (imageLower.contains(QRegularExpression(QStringLiteral("fedora|bluefin|ublue-os/fedora|fedoraproject\\.org/fedora")))) {
        ...
        QString message = QStringLiteral(
            ...
            "manager.");
        QString command = QStringLiteral("konsole --workdir %1 -e bash -c \"echo '%2'; read -n 1\"").arg(homeDir, message);
        bool success;
        runCommand(command, success);
        return success;
    }
    ...
    QString fullCmd = QStringLiteral("distrobox enter %1 -- %2; echo ''; echo 'Press any key to close this terminal...'; read -n 1").arg(name, installCmd);
    QString command = QStringLiteral("konsole --workdir %1 -e bash -c '%2'").arg(homeDir, fullCmd);
